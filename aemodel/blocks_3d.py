...
from aemodel.layers.mconv3d import MaskedConv3d
...
class GRN(nn.Module):
    ...
    def __init__(self, dim):
        super().__init__()
        self.gamma = nn.Parameter(torch.zeros(1, 1, 1, dim)).cuda(0)
        self.beta = nn.Parameter(torch.zeros(1, 1, 1, dim)).cuda(0)
...
class DownsampleBlock(BaseBlock):
    ...
    def __init__(self, channel_in, channel_out, activation_fn, stride, use_bn=True, use_bias=False):
        ...
        # torch.nn.init.xavier_uniform_(self.conv1a.weight, gain=torch.nn.init.calculate_gain('leaky_relu'))

        self.conv1b = MaskedConv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=3,
                                   padding=1, stride=1, bias=use_bias)
        # torch.nn.init.xavier_uniform_(self.conv1b.weight, gain=torch.nn.init.calculate_gain('leaky_relu'))

        self.conv2a = nn.Conv3d(in_channels=channel_in, out_channels=channel_out, kernel_size=1,
                                padding=0, stride=stride, bias=use_bias)
        # torch.nn.init.xavier_uniform_(self.conv2a.weight, gain=torch.nn.init.calculate_gain('leaky_relu'))
