'''
使用channel和时间上的权重，嵌入空间上时间特征维度为1
'''
import torch
...
class Encoder(BaseModule):
    ...
    def __init__(self, input_shape, code_length, layers = [3, 4]):
        ...
        self.conv = nn.Sequential(
            ...
            DownsampleBlock(channel_in=64, channel_out=64, activation_fn=activation_fn, stride=(1, 1, 2))
        )

        self.deepest_shape = (64, t // 4, h // 8, w // 8)
        #
        # # FC network
    ...
    def __init__(self, code_length, deepest_shape, output_shape, layers = [3, 4]):
        ...
        self.conv = nn.Sequential(
            UpsampleBlock(channel_in=dc, channel_out=64,
                          activation_fn=activation_fn, stride=(1, 1, 2), output_padding=(0, 0, 1)),
            # stride=(1,2,2)->(2,2,2) padding (0,1,1)_>(1,1,1)
...
class AeMultiOut(BaseModule):
    ...
    def __init__(self, input_shape, code_length, layers = [2,3,4]):
        ...
        self.vw = VarWght(is_app=True).to(next(self.decoder.parameters()).device)
        self.init_params()
